part3 <- function(FNIII,fc){
  # FNIII <- paste(FN,'/',FN1[4],sep='')
  ...
  # if(length(for_data)==0){
  #   out.put <- data.frame(rt=pos$rt,rawmz=pos$rawmz,X=pos$X,subclass=pos$subclass,
  #               Total.C=pos$Total.C,Total.Uns=pos$Total.Uns,Title=pos$Title,
  #               mz=pos$mz,Adduct=pos$Adduct,Chain=pos$Chain,cluster=pos$cluster,
  #               score.match=pos$score.match,score.ratio=pos$score.ratio,scale.score=0.5,
  #               label='NA')
  #   write.csv(out.put,paste(gsub('.rda','',FNIII),'_PHSI.csv',sep=''))
  # }
  # 开始PHS 2 -----------------------------------------------------------------
