...
def retention_time_inference(
    ...
    print("Start RT reliability assessment workflow\n开始RT可信度评估流程")
    ...
    if not os.path.exists(INPUT_CSV):
        print("Step 0 | Input RT assessment table not found; regenerating from files ending with MSMS_EQ2.csv\nStep 0 | 未找到输入RT判断表，开始从MSMS_EQ2.csv结尾文件重新生成")
        ...
        print(f"Step 0 | Generated new RT assessment input table {INPUT_CSV}\nStep 0 | 已生成新的RT判断输入表 {INPUT_CSV}")

    print("Step 1 | Reading input table\nStep 1 | 正在读取输入表格")
    df = pd.read_csv(INPUT_CSV)
    print(f"Step 1 | Input table loaded, record count {len(df)}\nStep 1 | 输入表格读取完成，记录数 {len(df)}")
    ...
    print("Step 2 | Parsing total_chain_name\nStep 2 | 正在解析 total_chain_name")
    df = add_lipid_features(df)

    print("Step 3 | Performing ECN rule-based RT inference\nStep 3 | 正在进行ECN规则RT推断")
    df = calculate_rule_based_rt_score(df)

    print("Step 4 | Combining RT assessment scores\nStep 4 | 正在整合RT判断得分")
    ...
    print("Step 5 | Saving result table\nStep 5 | 正在保存结果表格")
    df.to_csv(output_csv, index=False, encoding="utf-8-sig")

    print(f"Done. RT assessment results saved to {output_csv}\n完成，RT判断结果已保存到 {output_csv}")
