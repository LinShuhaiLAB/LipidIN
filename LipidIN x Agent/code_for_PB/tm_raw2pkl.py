...
def infer_acquisition_from_filter(filter_string, ms_order, isolation_width):
    ...
    if ms_order == 2:
        ...
        if is_finite_number(isolation_width) and float(isolation_width) >= 4.0:
            return "DIA_OR_AIF"
        # 窄隔离窗口(<4 Da)的产物离子扫描即使缺少 Thermo " d " 标记，
        # 也视为 DDA 型可用数据（母离子有效性由 precursor_ms2_count 单独把关）。
        if is_finite_number(isolation_width) and float(isolation_width) < 4.0:
            return "DDA"
        return "UNKNOWN_MS2"
    return "MS1"
def detect_acquisition_format_with_reader(reader, max_spectra=None):
    format_counts = {}
    ms1_count = 0
    ms2_count = 0
    ...
    try:
        for index, scan_number in enumerate(reader.scan_numbers(), start=1):
            if max_spectra is not None and index > max_spectra:
                break
...
def detect_acquisition_format(file_path, max_spectra=None):
    try:
...
def write_ms1_centroid_cache(cache_path, retention_times, mz_chunks, intensity_chunks):
    ...
    return cache_path
# 母离子精修默认开关：本类数据的 DDA 母离子 m/z 被量化到 0.1 Da 网格
# (trailer 无精确单同位素质量)，偏差可达 ±0.05Da(~90ppm) 超搜库容差导致匹配不上库。
# 设环境变量 PB_REFINE_PRECURSOR=0 可关闭（用于修复前后对比）。
REFINE_PRECURSOR_DEFAULT = os.environ.get("PB_REFINE_PRECURSOR", "1") != "0"


def refine_precursor_mz(nominal_mz, isolation_width, ms1_mz, ms1_int,
                        half_window_floor=0.05):
    """把(可能被量化的)DDA 母离子 m/z 精修到隔离窗内 MS1 去同位素最强峰的精确 m/z。

    在 nominal ± max(隔离半宽, floor) 窗口内取最强 MS1 单同位素峰。窗口内无峰则返回原值。
    """
    if ms1_mz.size == 0 or not math.isfinite(nominal_mz):
        return nominal_mz
    if math.isfinite(isolation_width) and isolation_width > 0:
        half = isolation_width / 2.0
    else:
        half = 0.5
    half = max(half, half_window_floor)
    mask = (ms1_mz >= nominal_mz - half) & (ms1_mz <= nominal_mz + half)
    if not mask.any():
        return nominal_mz
    idx = np.nonzero(mask)[0]
    best = idx[int(np.argmax(ms1_int[idx]))]
    return float(ms1_mz[best])
...
def parse_raw_dda_file(
    ...
    cache_mz = []
    cache_intensity = []
    if refine_precursor is None:
        refine_precursor = REFINE_PRECURSOR_DEFAULT
    last_ms1_mono_mz = np.empty(0, dtype=np.float64)
    last_ms1_mono_int = np.empty(0, dtype=np.float64)
    refined_count = 0
    refine_shift_abs_sum = 0.0
        ...
        for scan_number in reader.scan_numbers():
            ...
            try:
                ...
                if ms_order == 1:
                    ...
                    # 缓存本 MS1 去同位素峰，供随后 MS2 母离子精修用（隔离窗内最强峰）
                    if refine_precursor and monoisotopic_peaks:
                        last_ms1_mono_mz = np.fromiter(
                            (p["mz"] for p in monoisotopic_peaks),
                            dtype=np.float64, count=len(monoisotopic_peaks))
                        last_ms1_mono_int = np.fromiter(
                            (p["intensity"] for p in monoisotopic_peaks),
                            dtype=np.float64, count=len(monoisotopic_peaks))
                else:
                    precursor_mz = reader.get_precursor_mz(scan_number, event)
                    if refine_precursor:
                        refined = refine_precursor_mz(
                            precursor_mz, reader.get_isolation_width(event),
                            last_ms1_mono_mz, last_ms1_mono_int)
                        if abs(refined - precursor_mz) > 1e-4:
                            refined_count += 1
                            refine_shift_abs_sum += abs(refined - precursor_mz)
                        precursor_mz = refined
                    ms2_count += 1
    ...
    if refine_precursor and ms2_count > 0:
        avg_shift = refine_shift_abs_sum / refined_count if refined_count else 0.0
        warnings.append(
            f"母离子精修：{refined_count}/{ms2_count} 个 MS2 母离子按隔离窗内 MS1 最强峰精修"
            f"（平均位移 {avg_shift:.4f} Da）")
    cache_written = None
    ...
    return {
        ...
        "precursor_refined_count": refined_count,
        "warnings": warnings,
    }
...
def convert_one_file_to_standard_pkl(file_path, root_folder, MS2_filter, output_folder=None):
...
def process_single_file(task_args):
    ...
    try:
        ...
        log_info(
            f"文件处理完成 {file_name}，MS1_MSMS记录数={result.get('ms1_msms_records')}，MS1_only记录数={result.get('ms1_only_records')}"
        )
        return {"file": file_path, "status": "success", "result": result}
    except Exception as e:
        error_text = traceback.format_exc(limit=10)
        log_error(f"处理文件失败 {file_name}，原因={str(e)}")
...
def process_raw_files_batch(folder_path, MS2_filter=0.01, max_workers=None, recursive=True, output_folder=None):
    ...
    log_info(f"全部文件处理完成，成功={success_count}，失败={failed_count}，汇总={summary_path}")
    return summary
