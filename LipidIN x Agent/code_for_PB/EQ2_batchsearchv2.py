...
def strip_dtl_prefix(label):
    ...
    return text
# JCR 豁免：PI / PE-O / PC-O 在负模式主要靠 FA 羧酸阴离子(+头基)鉴定，其二级谱杂峰多、
# 强度分散，强度加权 JCR 天然偏低(实测 PI(34:2) 正确链 0.145、PE-O 40:7=0.117、38:7=0.103)，
# 标准 0.2 门会把这些高丰度不饱和物质全部误杀进 unannotated。对这几类用更低的豁免阈值，
# 并把靠豁免留下的注释标为低可信度；其它类维持原 JCR 门不变。
JCR_EXEMPT_SUBCLASSES = ("PI", "PE-O", "PC-O")
JCR_EXEMPT_CUTOFF = 0.1
JCR_EXEMPT_NOTE = "低可信度(JCR豁免<0.2)"


def postprocess_results_before_output(results_df, similarity_engine,
                                      ppm_err2=20, da_err2=0.01,
                                      jcr_cutoff=0.4, jcr_exempt_cutoff=JCR_EXEMPT_CUTOFF):
    if results_df is None or results_df.empty:
        return results_df
    ...
    # 标准 JCR 门 + PI/PE-O/PC-O 豁免（见文件内 JCR_EXEMPT_* 常量说明）
    jcr_num = results_df["jcr_similarity"]
    subclass_of_label = results_df["label"].astype(str).str.split(" ", n=1).str[0]
    is_exempt = subclass_of_label.isin(JCR_EXEMPT_SUBCLASSES)
    pass_normal = jcr_num > float(jcr_cutoff)
    pass_exempt = is_exempt & (~pass_normal) & (jcr_num > float(jcr_exempt_cutoff))

    # 低可信度备注：仅靠豁免留下的行标注，其它行留空
    results_df["annotation_confidence"] = np.where(
        pass_exempt.to_numpy(), JCR_EXEMPT_NOTE, ""
    )

    results_df = results_df.loc[pass_normal | pass_exempt].copy()
    results_df.reset_index(drop=True, inplace=True)

    n_removed_by_jcr = n_before_jcr_filter - len(results_df)
    n_exempt = int(pass_exempt.sum())
    if n_exempt:
        print(f"✓ JCR 豁免保留(低可信度, JCR>{jcr_exempt_cutoff}): {n_exempt}")
...
def merge_eq2_to_rt_ccs_aligned_table(
    ...
    for csv_file in csv_files:
        ...
        if sub_name_col not in df.columns:
            print(f"⚠ {csv_file.name} 缺少 {sub_name_col} 列，该文件的亚链名将留空")
            df[sub_name_col] = ""
        ...
        if missing_check_cols:
            print(f"⚠ {csv_file.name} 缺少 {missing_check_cols}，跳过头基/脂肪链过滤")
        else:
            ...
            print(
                f"✓ {csv_file.name} 头基/脂肪链过滤：{n_before_check_filter} -> {len(df)}"
            )
        ...
        if df.empty:
            print(f"⚠ {csv_file.name} 过滤后无剩余记录，跳过")
            continue
