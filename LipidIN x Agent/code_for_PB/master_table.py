...
# 某些类(如负模式 PI/LPE)在 HG.csv 无头基碎片参考 → EQ2 给 headgroup_check='无需判断'。
# 这是“不适用/无参考可查”，不是失败，理应被接受（甚至比“缺失=搜了没找到”更该接受）。
# 原白名单漏了它，导致 PI/LPE 虽 MS2 注释成功却在 load_eq2_rows 被全部过滤。
CHECK_NOT_APPLICABLE = "无需判断"


CHECK_ACCEPTED_OR_MISSING = CHECK_ACCEPTED + (CHECK_MISSING, CHECK_NOT_APPLICABLE)
...
def load_eq2_rows(pkl_dir, lines):
    ...
    log(f"_EQ2.csv 通过碎片双检并完成RT判断的记录 {len(merged)}")
...
def _load_or_build_library_entries(library_pkl):
    ...
    if os.path.exists(cache_path) and os.path.getmtime(cache_path) >= lib_mtime:
        try:
            ...
            if cached.get("version") == MS1_INDEX_CACHE_VERSION and cached.get("lib_mtime") == lib_mtime:
                log(f"命中 MS1 库索引缓存 {os.path.basename(cache_path)}，跳过整库解析")
                return cached["entries"]
            log("MS1 库索引缓存版本或来源不匹配，重建")
        except Exception as e:
            log(f"MS1 库索引缓存不可用，重建 {type(e).__name__}: {e}")

    log("构建 MS1 库索引（首次较慢：需载入并解析整个谱图库）")
    ...
    try:
        ...
        log(f"MS1 库索引已缓存 {cache_path}（下次直接复用）")
    except Exception as e:
        log(f"MS1 库索引缓存写入失败（不影响本次结果）{type(e).__name__}: {e}")
...
def build_ms1_library_index(library_pkl, lines):
    ...
    log(f"库唯一条目 {len(entries)}（已限定在有拟合线的 {entries['subclass'].nunique()} 个 subclass 内）")
...
def load_unannotated_rows(pkl_dir, entries, lines, ppm_err1, da_err1):
    ...
    log(f"_unannotated.csv 经 MS1 搜库并通过RT拟合筛选的记录 {len(result)}")
...
def load_ms1_only_rows(pkl_dir, entries, lines, ppm_err1, da_err1):
    ...
    log(f"MS1_only.pkl 经 MS1 搜库并通过RT拟合筛选的记录 {len(result)}")
...
def drop_ms1_rows_confirmed_by_ms2(df):
    ...
    log(f"因已有 MS2 证据而丢弃的 MS1 推断行 {int(redundant.sum())}")
...
def build_master_table(
    ...
    # ms2_only=True：只保留有 MS2 证据的注释(EVIDENCE_MS2)，跳过 Step4(_unannotated 的
    # MS1 推断) 与 Step5(MS1_only 的 MS1 推断)。此时 include_ms1_only 无效。
    if ms2_only:
        include_ms1_only = False
    if rt_checked_csv is None:
        rt_checked_csv = os.path.join(pkl_dir, "rt_ccs_aligned_table_RT_checked.csv")

    log("Step 1 | 按 subclass 拟合 RT ~ ECN 直线")
    lines = fit_rt_lines(rt_checked_csv)

    summary = pd.DataFrame(lines).T.reset_index().rename(columns={"index": "subclass"})
    print(summary.round(4).to_string(index=False))

    log("Step 2 | 读取 _EQ2.csv 并直接判断")
    eq2_rows = load_eq2_rows(pkl_dir, lines)

    log("Step 3 | 构建 MS1 级搜库索引")
    entries = build_ms1_library_index(library_pkl, lines)

    if ms2_only:
        log("Step 4 | (ms2_only) 跳过 _unannotated.csv 的 MS1 推断")
        unannotated_rows = pd.DataFrame(columns=MASTER_COLUMNS)
    else:
        log("Step 4 | _unannotated.csv 做 MS1 搜库 + RT 拟合判断")
        unannotated_rows = finalize_ms1_rows(load_unannotated_rows(pkl_dir, entries, lines, ppm_err1, da_err1))

    if include_ms1_only:
        log("Step 5 | MS1_only.pkl 做 MS1 搜库 + RT 拟合判断")
    ...
    log(f"合并后原始记录 {len(combined)}")

    combined = drop_ms1_rows_confirmed_by_ms2(combined)

    log("Step 7 | 按 sub_chain_name 去重")
    ...
    log(f"总表已保存 {output_path}")
    log(f"总表记录 {len(master)}")
    print("\n证据来源分布:")
